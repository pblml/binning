    ...
    for x in bins1.keys(): #bins
        if x in ["".join(["bin", str(i)]) for i in bin]:
            for idx, (i, j) in enumerate(zip(bins1[x].values(), bins2[x].values())): #idx=t
                    ax1.add_patch(
                            patches.Rectangle((idx-0.5, min(i)), 0.5, max(i)-min(i), color=(1.0, 0.84, 0.17), alpha=0.5)
                            )
                    ax1.add_patch(
                            patches.Rectangle((idx, min(j)), 0.5, max(j)-min(j), color=(0.35, 0.36, 0.91), alpha=0.5)
                            )
